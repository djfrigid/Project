...
def print_menu(exits, room_items, inv_items):
    ...
    for i in range(0, len(room_items),1):
        itemID = room_items[i]["id"]
        itemName = room_items[i]["name"]
        print("TAKE " + itemID.upper() + " to take a " + itemName)
    
    for j in range(0, len(inv_items),1):
        itemID = inv_items[j]["id"]
        itemName = inv_items[j]["name"]
        print("DROP " + itemID.upper() + " to drop your " + itemName)
